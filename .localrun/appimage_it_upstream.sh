...
for system_to_build_for in $build_for ; do
    ...
    system_to_build_for=$(echo "$system_to_build_for_orig" 2>/dev/null|tr ':' '_' 2>/dev/null)"_appimage_upstream"
    ...
    cp -v utox_upstream.json $_HOME_/"$system_to_build_for"/workspace/utox.json
...
cp -av /workspace/uTox/build2/uTox*x86_64.AppImage /artefacts/uTox_x86_64.upstream.AppImage
...
ls -alh *_appimage_upstream/artefacts/
